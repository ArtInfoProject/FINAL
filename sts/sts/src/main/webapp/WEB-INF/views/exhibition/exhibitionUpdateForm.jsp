...
<body>
	<div id="writeBody">
		...
		<%-- 	${detail} --%>
		<form
			action="/exhibition/update?exhibition_idx=${detail.exhibition_idx}"
			method="POST">
			...
			<div>
				...
				<textarea name="exhibition_contents" id="exhibition_contents"
					rows="20" cols="50">${detail.exhibition_contents }</textarea>
			</div>
			<div>
				<label>전시회 image : </label>
				<!-- 			<input type="file" id="exhibition_img" -->
				<%-- 				name="exhibition_img" accept=".jpg,.png" value="${detail.exhibition_img}"> --%>
				<input type="text" id="exhibition_img" name="exhibition_img"
					value="${detail.exhibition_img }">
				<%-- 				${detail.exhibition_img} --%>
			</div>
